...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:for-each-group select="//anchor[@type='collate']/following-sibling::node()" group-starting-with="anchor[@type='collate']">
            ...
            <!-- <xsl:result-document href="../msCollChunks-May2023/{current()/@xml:id}/{substring-before(tokenize(ancestor::xml/base-uri(), '/')[last()], '.')  ! substring-before(., '_')}_{current()/@xml:id}.xml" method="xml" indent="no">
-->               
